"""
Thanks to Kneesocks. I just added the AnonArmy Check
"""
import re
import html
import asyncio
...
@app.on_message(cmd(['einfo', 'spamcheck', 'sw', 'spamwatch', 'aa', 'anon', 'deai', 'spb', 'spamprotection', 'cas', 'combot', 'rose', 'sibyl']) & filters.user(AUTH))
...
async def get_spamwatch(entity):
    async with session.get(f'https://api.spamwat.ch/banlist/{entity}', headers={'Authorization': f'Bearer {SPAMWATCH_API}'}) as resp:
        try:
...
async def get_sibyl(client, entity):
    new_message = await client.send_message('missrose_bot', f'/fbanstat {entity} 5dcc7db0-0fbc-46c9-bbe7-5700ef47a0d5')
    identifier = (new_message.chat.id, new_message.message_id)
    conversation_hack[identifier] = None
    while not conversation_hack[identifier]:
        await asyncio.sleep(0.5)
